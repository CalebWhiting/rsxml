 ...
 */
public class XMLFieldCount extends XMLClassFilter {
	...
	public boolean match(ClassNode node) {
		...
		int value = getFieldCount(XMLProcessor.getLibrary().get(owner), desc);
		return mode.isNumericMatch(value, amount);
	}

	private int getFieldCount(ClassNode c, String desc) {
		int i = 0;
		for (FieldNode fn : c.fields) {
			if ((fn.access & Opcodes.ACC_STATIC) == 0 && (desc.length() == 0 || desc.equals(fn.desc))) {
				i++;
			}
		}
		return i;
